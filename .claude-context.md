...
- Implemented 14 mobile-native features from saved plan (`serene-rolling-treasure.md`)
- All changes in `public/index.html` only (no server changes)
- **Bottom Action Sheet**: Slide-up sheet replaces context menu on mobile, swipe handle to dismiss
- **Long-Press to Select**: 500ms touch-hold enters selection mode with haptic feedback
- **Pull-to-Refresh**: Pull down at scroll top refreshes directory listing
- **FAB (Floating Action Button)**: Bottom-right + button → Upload Files, New Folder, New File, Take Photo
- **Swipe Navigation**: Edge swipe left = back, right = forward with visual indicators
- **Web Share API**: Native `navigator.share` for single files when available
- **Bottom Tab Bar**: Persistent 4-tab nav (Files, Favorites, Upload, Settings)
- **Camera Upload**: `capture="environment"` file input, accessible from FAB menu
- **Haptic Feedback**: `navigator.vibrate` on selection, FAB, bottom sheet interactions
- **Pinch-to-Zoom**: Two-finger zoom + double-tap toggle on images in media overlay
- **Compact List View**: Dense 40px single-line rows — third view mode (Grid/Details/Compact)
- **Edge Swipe Indicators**: Visual blue gradient feedback during swipe gestures
- **Mobile Selection Mode**: Taps toggle selection when items are already selected
- All features gated behind `isMobile()` check (< 768px), desktop untouched
- `public/index.html`: 5825 → 6876 lines (+1051 lines)
- Browser-tested at 375x812 (iPhone) and 1280x800 (desktop) — all features verified
